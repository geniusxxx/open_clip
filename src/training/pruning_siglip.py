...
class PruningManager:
    ...
    def __init__(
        ...
        self.config = {
            ...
            'round_to': 8,
            'bottleneck': True, # timm vit限制了mlp的输入输出通道数相等，所以只能默认设置为true
            'pruning_done': False,
            'isomorphic': False,
    ...
    def _prepare_model(self):
        ...
        def forward(self_module, x):
            ...
            q, k = self_module.q_norm(q), self_module.k_norm(k)
            
            if hasattr(self_module, 'fused_attn') and self_module.fused_attn:
                x = F.scaled_dot_product_attention(q, k, v, dropout_p=self_module.attn_drop.p)
            else:
        ...
        if hasattr(self.model, 'patch_embed'):
            ignored_layers.append(self.model.patch_embed)

        # 5. 保护attention pool层和最后一个block的fc2
        if hasattr(self.model.visual.trunk, 'attn_pool'):
            attn_pool = self.model.visual.trunk.attn_pool
            ignored_layers.append(attn_pool)
            ignored_layers.append(attn_pool.q)
            ignored_layers.append(attn_pool.kv)
            if hasattr(attn_pool, 'proj'):
                ignored_layers.append(attn_pool.proj)
            if hasattr(attn_pool, 'mlp'):
                ignored_layers.append(attn_pool.mlp)
                ignored_layers.append(attn_pool.mlp.fc1)
                ignored_layers.append(attn_pool.mlp.fc2)

        # 6. 处理attention和mlp层
        for m in self.model.modules():
            if isinstance(m, timm.models.vision_transformer.Attention) and not isinstance(m, timm.models.vision_transformer.AttentionPoolLatent):
            ...
            if isinstance(m, timm.models.vision_transformer.Mlp):
                # 当bottleneck=True时，忽略所有fc2
                if self.config['bottleneck']:
                    # print(f"\n添加bottleneck fc2 id: {id(m.fc2)}")
